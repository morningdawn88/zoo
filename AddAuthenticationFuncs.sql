...
create view users_view as select users.first_name,users.last_name,users.username,
   users.division,users.department,users.password from users;

create or replace rule ins_user as on insert to users_view
  do instead insert into users(first_name,last_name, username,division,department,enc_pass,salt)
  values (new.first_name,new.last_name,new.username,new.division,new.department,
